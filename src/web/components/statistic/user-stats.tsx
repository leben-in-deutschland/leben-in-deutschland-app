    ...
    return (
        <div className="grid gap-y-5">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
                <NumberProgress title={translation.skipped} count={skipped} numberColor={""} isPressable={skipped > 0} onPress={() => router.push("/prepare?action=skipped")} icon={<SkipIcon size={14} />} />
                <NumberProgress title={translation.flagged} count={flagged} numberColor={""} isPressable={flagged > 0} onPress={() => router.push("/prepare?action=flagged")} icon={<FlagOutlineIcon size={14} />} />
                <NumberProgress title={translation.correct} count={correct} numberColor={"green-500"} isPressable={correct > 0} onPress={() => router.push("/prepare?action=correct")} icon={<CheckCircleIcon size={14} />} />
                <NumberProgress title={translation.incorrect} count={incorrect} numberColor={"red-700"} isPressable={incorrect > 0} onPress={() => router.push("/prepare?action=incorrect")} icon={<XCircleIcon size={14} />} />
